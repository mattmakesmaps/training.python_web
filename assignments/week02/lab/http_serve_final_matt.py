...
def make_time():
    """
    Return an html page with the current time.
    From make_time.py
    """
...
def resolve_uri(inuri):
    ...
    # Inspect paths and handle based on time endpoint, file, or folder.
    # Time Path
    print 'In URI: %s' % inuri
    if inuri == 'time-page':
        body = make_time()
...
def parse_request(request):
    ...
    if uriHeader[0] == 'GET' and 'HTTP' in uriHeader[2]:
        ...
        response_body = 'ERROR: Request Not Implemented. Please send an HTTP GET Request.'
        response = make_response(response_body,'501 Not Implemented')
        return response
...
#        headers = re.findall(r"(?P<name>.*?): (?P<value>.*?)\r\n", data)
        ...
        internal_server_error = make_response(error_text,'500 Internal Server Error')
        if client:
            client.send(internal_server_error)
            client.close()
